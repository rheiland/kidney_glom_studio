...
#------------------------------------------------------------------------------
    ...
    def plot_substrate(self, frame):
        ...
        # plt.close("all")
        if not self.plot_window_exists:
            self.figure = plt.figure()
            self.canvas = FigureCanvasQTAgg(self.figure)
            self.canvas.setStyleSheet("background-color:transparent;")
            # self.ax0 = self.figure.add_subplot(111)
            self.myscroll.setWidget(self.canvas) # self.config_params = QWidget()
            self.plot_window_exists = True
        else:
            self.figure = plt.figure()
    ...
    #    print('time=' + root.find(".//current_time").text)
        ...
        if (False):   # fixed range 
            ...
            try:
                ...
                substrate_plot = plt.contourf(xgrid, ygrid, M[self.field_index, :].reshape(self.numy,self.numx), num_contours, cmap='viridis')  # self.colormap_dd.value)
            except:
                contour_ok = False
        ...
        if (contour_ok):
            ...
            cbar = self.figure.colorbar(substrate_plot)
